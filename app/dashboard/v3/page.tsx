...
// ═══════════════════════════════════════════════════════════════════
//  PIILK Dashboard V3
//  V2 전체 기능 + Meta Marketing API 자동 연동
//  ├─ 캠페인별 CPL / Spend / CTR 실시간 표시
//  ├─ 일별 Spend 추이 그래프
//  └─ Main vs Quiz variant ROI 비교
// ═══════════════════════════════════════════════════════════════════
...
/* ═══════════════════════════════════════════════════════════════════
   META ADS SECTION COMPONENT
   ─ 자동 API 연동 + 캠페인별 CPL + 일별 Spend 그래프
═══════════════════════════════════════════════════════════════════ */
...
function MetaAdsSection({
  ...
  const [metaLevel, setMetaLevel]     = useState<'campaign' | 'ad'>('campaign');
  const [metaDateFrom, setMetaDateFrom] = useState('');
  const [metaDateTo, setMetaDateTo]     = useState('');
  const [spendChartMetric, setSpendChartMetric] = useState<'spend' | 'clicks' | 'pageViews'>('spend');
  ...
  const fetchMeta = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        preset: metaPreset,
        level: metaLevel,
        ...(metaPreset === 'custom' && metaDateFrom ? { from: metaDateFrom } : {}),
        ...(metaPreset === 'custom' && metaDateTo   ? { to:   metaDateTo   } : {}),
      });
      const res = await fetch(`/api/dashboard/meta-insights?${params}`);
      const data: MetaInsightsData = await res.json();
      if (!data.success) throw new Error(data.error || 'API error');
      setMetaData(data);
    } catch (err: any) {
      setMetaError(err.message || '알 수 없는 오류');
    } finally {
      setMetaLoading(false);
    }
  }, [metaPreset, metaLevel, metaDateFrom, metaDateTo]);

  useEffect(() => { fetchMeta(); }, [fetchMeta]);

  // ─── CPL 계산: 우리 Supabase submits 기준 ───
  const ourCPL = useMemo(() => {
    if (!metaData?.summary?.spend || !metaData?.period) return null;
    const { since, until } = metaData.period;
    const metaSubmits = currentParticipants.filter(p => {
      const d = toNYCDateStr(p.signed_up_at);
      return d >= since && d <= until && normalizeUtmSource(p.utm_source) === 'meta';
    }).length;
    if (metaSubmits === 0) return null;
    return { submits: metaSubmits, cpl: metaData.summary.spend / metaSubmits };
  }, [metaData, currentParticipants]);

  // ─── Variant별 캠페인 분리 ───
  const campaignsByVariant = useMemo(() => {
    if (!metaData?.campaigns) return { main: [], type: [], unknown: [] };
    return {
      main:    metaData.campaigns.filter(c => c.variant === 'main'),
      type:    metaData.campaigns.filter(c => c.variant === 'type'),
      unknown: metaData.campaigns.filter(c => c.variant === 'unknown'),
    };
  }, [metaData]);

  // ─── Daily Spend 차트 ───
  const SpendChart = () => {
    if (!metaData?.dailySpend?.length) return null;
    const data = metaData.dailySpend;
    const maxVal = Math.max(...data.map(d => d[spendChartMetric] as number), 1);
    const totalSpend = data.reduce((s, d) => s + d.spend, 0);

    const metricLabel: Record<string, string> = { spend: 'Spend ($)', clicks: 'Clicks', pageViews: 'LP Views' };
    const metricColor: Record<string, string> = { spend: 'bg-blue-400', clicks: 'bg-amber-400', pageViews: 'bg-emerald-400' };
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider">Daily Ad Spend</h3>
            <span className="text-[10px] text-zinc-500 font-mono">Total ${totalSpend.toFixed(2)}</span>
          </div>
          <div className="flex gap-1 bg-zinc-800 rounded-lg p-0.5">
            {(['spend', 'clicks', 'pageViews'] as const).map(m => (
              <button
                key={m}
                onClick={() => setSpendChartMetric(m)}
                className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-all ${spendChartMetric === m ? 'bg-white text-black' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                {metricLabel[m]}
              </button>
            ))}
        ...
        <div className="relative h-40 sm:h-48">
          <div className="absolute left-0 top-0 bottom-5 w-10 flex flex-col justify-between text-[9px] text-zinc-600 font-mono">
            <span>{spendChartMetric === 'spend' ? `$${maxVal.toFixed(0)}` : maxVal}</span>
            <span>{spendChartMetric === 'spend' ? `$${(maxVal / 2).toFixed(0)}` : Math.round(maxVal / 2)}</span>
            <span>0</span>
          </div>
          <div className="absolute left-12 right-0 top-0 bottom-5 flex items-end gap-[2px]">
            {data.map((d) => {
              const val = d[spendChartMetric] as number;
              const height = maxVal > 0 ? (val / maxVal) * 100 : 0;
              const isToday = d.date === getNYCDate(0);
              return (
                <div key={d.date} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                  <div className="absolute -top-9 left-1/2 -translate-x-1/2 bg-zinc-800 border border-zinc-700 text-white text-[9px] px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                    <p className="font-bold">{d.date}</p>
                    <p>💰 ${d.spend.toFixed(2)}</p>
                    <p>👆 {d.clicks} clicks</p>
                    <p>🏠 {d.pageViews} LP views</p>
                    <p>🎯 {d.leads} leads</p>
                  </div>
                  <div
                    className={`w-full rounded-t-sm transition-all duration-500 ${isToday ? 'bg-emerald-400' : metricColor[spendChartMetric]} group-hover:opacity-80`}
                    style={{ height: `${Math.max(height, val > 0 ? 2 : 0)}%` }}
                  />
                  {(data.length <= 14 || data.indexOf(d) % Math.ceil(data.length / 8) === 0) && (
                    <span className={`text-[7px] mt-1 font-mono ${isToday ? 'text-emerald-400 font-bold' : 'text-zinc-600'}`}>
                      {d.date.slice(5)}
                    </span>
                  )}
                </div>
        ...
        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-zinc-800/50 text-[10px] text-zinc-500">
          <span>Period: <span className="text-zinc-300 font-medium">{data.length}d</span></span>
          <span>Avg/day: <span className="text-zinc-300 font-medium">${(totalSpend / Math.max(data.length, 1)).toFixed(2)}</span></span>
          <span>Peak: <span className="text-blue-400 font-medium">${Math.max(...data.map(d => d.spend)).toFixed(2)}</span></span>
        </div>
      </div>
    );
  };
  ...
  const SummaryCards = () => {
    ...
    const cards = [
      { label: 'Total Spend',  value: `$${s.spend.toFixed(2)}`,          sub: `$${(s.spend / Math.max(metaData.dailySpend.length, 1)).toFixed(2)}/day`, color: 'text-blue-400',    bg: 'bg-blue-950/30 border-blue-900/40'   },
      { label: 'Impressions',  value: s.impressions.toLocaleString(),     sub: `CPM $${s.cpm.toFixed(2)}`,                                              color: 'text-zinc-300',    bg: 'bg-zinc-900/60 border-zinc-800'      },
      { label: 'Link Clicks',  value: s.clicks.toLocaleString(),          sub: `CTR ${s.ctr.toFixed(2)}%`,                                              color: 'text-amber-400',   bg: 'bg-amber-950/30 border-amber-900/40' },
      { label: 'LP Views',     value: s.pageViews.toLocaleString(),       sub: `CPC $${s.cpc.toFixed(2)}`,                                              color: 'text-purple-400',  bg: 'bg-purple-950/30 border-purple-900/40'},
      { label: 'Meta Leads',   value: s.leads.toLocaleString(),           sub: s.cpl > 0 ? `CPL $${s.cpl.toFixed(2)}` : 'Meta reported',                color: 'text-emerald-400', bg: 'bg-emerald-950/30 border-emerald-900/40'},
      { label: 'Our CPL',      value: ourCPL ? `$${ourCPL.cpl.toFixed(2)}` : '—', sub: ourCPL ? `${ourCPL.submits} submits` : 'No meta signups',        color: 'text-rose-400',    bg: 'bg-rose-950/30 border-rose-900/40'   },
    ];
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
        {cards.map(c => (
          <div key={c.label} className={`border rounded-xl p-3 sm:p-4 ${c.bg}`}>
            <p className="text-[9px] text-zinc-500 uppercase tracking-widest font-semibold mb-1">{c.label}</p>
  ...
  // ─── Campaign Table ───
  const CampaignTable = ({ campaigns, title, accentColor }: { campaigns: MetaCampaign[]; title: string; accentColor: string }) => {
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-500">{campaigns.length} campaigns · ${totalSpend.toFixed(2)} total</span>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-zinc-800/60 bg-zinc-900/40">
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold">Campaign</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">Spend</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">Impr.</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">Clicks</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">CTR</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">LP Views</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">Leads</th>
                <th className="px-3 py-2 text-[9px] text-zinc-500 uppercase tracking-widest font-semibold text-right">CPL</th>
              </tr>
            </thead>
            <tbody>
              {campaigns.map((c) => {
                const spendPct = totalSpend > 0 ? (c.spend / totalSpend) * 100 : 0;
                const isExpanded = expandedCampaign === c.campaignId;
                const adsForCampaign = metaData?.ads?.filter(a => a.campaignName === c.campaignName) || [];
                ...
                return (
                  <>
                    <tr
                      ...
                      onClick={() => {
                        if (metaLevel === 'ad' && adsForCampaign.length > 0) {
                          setExpandedCampaign(isExpanded ? null : c.campaignId);
                        }
                      }}
                    >
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-2">
                          {metaLevel === 'ad' && adsForCampaign.length > 0 && (
                            <span className="text-zinc-600 text-[10px]">{isExpanded ? '▼' : '▶'}</span>
                          )}
                          <div>
                            <p className="text-xs text-white font-medium max-w-[200px] truncate" title={c.campaignName}>{c.campaignName}</p>
                            <div className="mt-1 h-1 bg-zinc-800 rounded-full overflow-hidden w-24">
                              <div className={`h-full ${accentColor} rounded-full`} style={{ width: `${spendPct}%` }} />
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-xs text-blue-400 text-right font-mono font-bold">${c.spend.toFixed(2)}</td>
                      <td className="px-3 py-2.5 text-xs text-zinc-400 text-right font-mono">{c.impressions.toLocaleString()}</td>
                      <td className="px-3 py-2.5 text-xs text-amber-400 text-right font-mono">{c.clicks}</td>
                      <td className="px-3 py-2.5 text-xs text-zinc-300 text-right font-mono">{c.ctr.toFixed(2)}%</td>
                      <td className="px-3 py-2.5 text-xs text-purple-400 text-right font-mono">{c.pageViews}</td>
                      <td className="px-3 py-2.5 text-xs text-emerald-400 text-right font-mono font-bold">{c.leads}</td>
                      <td className="px-3 py-2.5 text-right">
                        {c.cpl > 0
                          ? <span className={`text-xs font-mono font-bold ${c.cpl < 5 ? 'text-emerald-400' : c.cpl < 15 ? 'text-amber-400' : 'text-red-400'}`}>${c.cpl.toFixed(2)}</span>
                          : <span className="text-xs text-zinc-600">—</span>
                        }
                      </td>
                    </tr>
                    {/* ─── Ad-level expand ─── */}
                    {isExpanded && adsForCampaign.map(ad => (
                      <tr key={ad.adId} className="border-b border-zinc-800/20 bg-zinc-950/50">
                        <td className="px-3 py-2 pl-10">
                          <p className="text-[10px] text-zinc-400 max-w-[180px] truncate">↳ {ad.adName}</p>
                          <p className="text-[9px] text-zinc-600">{ad.adsetName}</p>
                        </td>
                        <td className="px-3 py-2 text-[10px] text-blue-300 text-right font-mono">${ad.spend.toFixed(2)}</td>
                        <td className="px-3 py-2 text-[10px] text-zinc-500 text-right font-mono">{ad.impressions.toLocaleString()}</td>
                        <td className="px-3 py-2 text-[10px] text-amber-300 text-right font-mono">{ad.clicks}</td>
                        <td className="px-3 py-2 text-[10px] text-zinc-400 text-right font-mono">{ad.ctr.toFixed(2)}%</td>
                        <td className="px-3 py-2 text-[10px] text-purple-300 text-right font-mono">{ad.pageViews}</td>
                        <td className="px-3 py-2 text-[10px] text-emerald-300 text-right font-mono">{ad.leads}</td>
                        <td className="px-3 py-2 text-[10px] text-right font-mono">
                          {ad.cpl > 0 ? <span className={ad.cpl < 5 ? 'text-emerald-300' : ad.cpl < 15 ? 'text-amber-300' : 'text-red-300'}>${ad.cpl.toFixed(2)}</span> : <span className="text-zinc-700">—</span>}
                        </td>
                      </tr>
                    ))}
                  </>
  ...
  // ─── Main vs Type ROI Comparison ───
  const VariantROI = () => {
    const { main, type } = campaignsByVariant;
    if (!main.length && !type.length) return null;

    const sum = (arr: MetaCampaign[], key: keyof MetaCampaign): number =>
      arr.reduce((s, c) => s + (Number(c[key]) || 0), 0);

    // 우리 Supabase 데이터와 매핑
    const since = metaData?.period.since || '';
    const until = metaData?.period.until || '';

    const mainSubmits = currentParticipants.filter(p => {
      const d = toNYCDateStr(p.signed_up_at);
      return d >= since && d <= until && normalizeUtmSource(p.utm_source) === 'meta';
    }).filter(p => (p as any)._variantTag === 'main' || !p.afterfeel_type).length;

    const typeSubmits = currentParticipants.filter(p => {
      const d = toNYCDateStr(p.signed_up_at);
      return d >= since && d <= until && normalizeUtmSource(p.utm_source) === 'meta' && p.afterfeel_type;
    }).length;

    const variants = [
      { label: 'Main Teaser', color: 'emerald', campaigns: main, submits: mainSubmits,
        spend: sum(main, 'spend'), clicks: sum(main, 'clicks'), pageViews: sum(main, 'pageViews'), leads: sum(main, 'leads') },
      { label: 'Quiz Type',   color: 'purple',  campaigns: type, submits: typeSubmits,
        spend: sum(type, 'spend'), clicks: sum(type, 'clicks'), pageViews: sum(type, 'pageViews'), leads: sum(type, 'leads') },
    ];
    ...
    return (
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 sm:p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider">Main vs Quiz — ROI Comparison</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {variants.map(v => {
            const cpl = v.submits > 0 ? v.spend / v.submits : 0;
            const cvr = v.clicks  > 0 ? (v.submits / v.clicks) * 100 : 0;
            const colorMap: Record<string, { bg: string; border: string; text: string; badge: string }> = {
              emerald: { bg: 'bg-emerald-950/30', border: 'border-emerald-900/40', text: 'text-emerald-400', badge: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' },
              purple:  { bg: 'bg-purple-950/30',  border: 'border-purple-900/40',  text: 'text-purple-400',  badge: 'bg-purple-500/20 text-purple-400 border-purple-500/30'   },
            };
            const c = colorMap[v.color];
            return (
              <div key={v.label} className={`${c.bg} border ${c.border} rounded-xl p-4`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-[10px] px-2 py-0.5 rounded border font-bold ${c.badge}`}>{v.label}</span>
                  <span className="text-[10px] text-zinc-500">{v.campaigns.length} campaigns</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { l: 'Spend',    v: `$${v.spend.toFixed(2)}`,          c: 'text-blue-400'   },
                    { l: 'Clicks',   v: v.clicks.toString(),                c: 'text-amber-400'  },
                    { l: 'LP Views', v: v.pageViews.toString(),             c: 'text-purple-400' },
                    { l: 'Submits',  v: v.submits.toString(),               c: c.text            },
                    { l: 'Our CPL',  v: cpl > 0 ? `$${cpl.toFixed(2)}` : '—', c: cpl > 0 && cpl < 10 ? 'text-emerald-400' : 'text-rose-400' },
                    { l: 'CVR',      v: cvr > 0 ? `${cvr.toFixed(1)}%` : '—', c: cvr > 3 ? 'text-emerald-400' : 'text-zinc-400' },
                  ].map(item => (
                    <div key={item.l} className="bg-black/20 rounded-lg p-2 text-center">
                      <p className={`text-sm sm:text-lg font-black ${item.c}`}>{item.v}</p>
                      <p className="text-[8px] text-zinc-500 uppercase tracking-widest mt-0.5">{item.l}</p>
                    </div>
                  ))}
                </div>
              </div>
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm sm:text-base font-bold text-white uppercase tracking-widest">Meta Ads</h2>
          <span className="text-[10px] px-2 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 font-bold">Live API</span>
          {metaData?.period && (
            <span className="text-[10px] text-zinc-500 font-mono hidden sm:inline">
              {metaData.period.since} → {metaData.period.until}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {/* Preset 버튼 */}
          <div className="flex gap-1 bg-zinc-800 rounded-lg p-0.5">
            {META_PRESET_OPTIONS.map(opt => (
              <button
                key={opt.key}
                onClick={() => setMetaPreset(opt.key)}
                className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-all ${metaPreset === opt.key ? 'bg-blue-500 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                {opt.label}
              </button>
            ))}
          ...
          <div className="flex gap-1 bg-zinc-800 rounded-lg p-0.5">
            {(['campaign', 'ad'] as const).map(l => (
              <button
                key={l}
                onClick={() => setMetaLevel(l)}
                className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-all capitalize ${metaLevel === l ? 'bg-zinc-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                {l}
              </button>
            ))}
          ...
          <button
            ...
            className="px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-400 text-xs hover:bg-zinc-800 hover:text-white disabled:opacity-50 flex items-center gap-1.5"
          >
            <svg className={`w-3.5 h-3.5 ${metaLoading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Custom Date Range */}
      <div className="flex items-center gap-2 flex-wrap">
        <label className="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold">Custom:</label>
        <input type="date" value={metaDateFrom} onChange={e => { setMetaDateFrom(e.target.value); if (e.target.value && metaDateTo) setMetaPreset('custom'); }}
          className="px-2 py-1 bg-zinc-800 border border-zinc-700 rounded-lg text-xs text-white focus:outline-none focus:border-zinc-600" />
        <span className="text-zinc-600 text-xs">→</span>
        <input type="date" value={metaDateTo} onChange={e => { setMetaDateTo(e.target.value); if (metaDateFrom && e.target.value) setMetaPreset('custom'); }}
          className="px-2 py-1 bg-zinc-800 border border-zinc-700 rounded-lg text-xs text-white focus:outline-none focus:border-zinc-600" />
        {metaPreset === 'custom' && <span className="text-[10px] px-2 py-0.5 rounded bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">Custom</span>}
      </div>
      ...
      {metaError && (
        <div className="bg-red-950/30 border border-red-900/50 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-red-400">Meta API 오류</p>
            <p className="text-xs text-red-300/70 mt-0.5">{metaError}</p>
            <p className="text-[10px] text-zinc-500 mt-2">
              .env에 <code className="bg-zinc-800 px-1 rounded text-zinc-300">META_ACCESS_TOKEN</code>과 <code className="bg-zinc-800 px-1 rounded text-zinc-300">META_AD_ACCOUNT_ID</code> 확인
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 animate-pulse">
              <div className="h-2 bg-zinc-800 rounded mb-2 w-16" />
        ...
        <>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Campaigns</h3>
              {metaLevel === 'ad' && (
                <p className="text-[10px] text-zinc-600">캠페인 클릭 → 광고 상세 펼치기</p>
              )}
            </div>
            {campaignsByVariant.main.length > 0 && (
              <CampaignTable campaigns={campaignsByVariant.main} title="Main Teaser Campaigns" accentColor="bg-emerald-500" />
            )}
            {campaignsByVariant.type.length > 0 && (
              <CampaignTable campaigns={campaignsByVariant.type} title="Quiz Type Campaigns" accentColor="bg-purple-500" />
            )}
            {campaignsByVariant.unknown.length > 0 && (
              <CampaignTable campaigns={campaignsByVariant.unknown} title="Other Campaigns" accentColor="bg-zinc-500" />
            )}
            {!metaData.campaigns.length && (
              <div className="text-center text-zinc-600 py-6 text-sm">해당 기간 캠페인 데이터 없음</div>
            )}
          </div>
        </>
      )}
...
export default function DashboardV3Page() {
  ...
  // View mode — ✅ 'meta' 탭 추가
  const [viewMode, setViewMode] = useState<'overview' | 'participants' | 'analytics' | 'meta'>('participants');
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (result.success) { setSupabaseData(result.supabase); setKlaviyoData(result.klaviyo); setLastUpdated(new Date().toLocaleTimeString('ko-KR')); }
  ...
  const fetchParticipants = useCallback(async () => {
    ...
    try {
      ...
      setParticipants({ klaviyo: kResult.success ? kResult.data : [], supabase: sResult.success ? sResult.data : [] });
      ...
      setOtherParticipants({ klaviyo: kOtherResult.success ? kOtherResult.data : [], supabase: sOtherResult.success ? sOtherResult.data : [] });
    } catch (err) { console.error(err); }
  ...
  useEffect(() => {
    if (authenticated) {
      setLoading(true); setSupabaseData(null); setKlaviyoData(null);
      setParticipants({ klaviyo: [], supabase: [] }); setAnalyticsData(null);
      fetchData(); fetchParticipants(); fetchAnalytics();
    }
  ...
  const dailySignups = useMemo(() => {
    ...
    currentParticipants.forEach(p => { const day = toNYCDateStr(p.signed_up_at); if (day) dayMap[day] = (dayMap[day] || 0) + 1; });
  ...
  const uniqueReasons  = useMemo(() => { const s = new Set<string>(); currentParticipants.forEach(p => { if (p.sub_reason) s.add(p.sub_reason); }); return Array.from(s).sort(); }, [currentParticipants]);
  const uniqueDomains  = useMemo(() => { const s = new Set<string>(); currentParticipants.forEach(p => { if (p.email?.includes('@')) s.add(p.email.split('@')[1].toLowerCase()); }); return Array.from(s).sort(); }, [currentParticipants]);
  const uniqueCountries= useMemo(() => { const s = new Set<string>(); currentParticipants.forEach(p => { if (p.country) s.add(p.country); }); return Array.from(s).sort(); }, [currentParticipants]);
  const uniqueCities   = useMemo(() => { const s = new Set<string>(); currentParticipants.forEach(p => { if (p.city) s.add(p.city); }); return Array.from(s).sort(); }, [currentParticipants]);
  const uniqueDevices  = useMemo(() => { const s = new Set<string>(); currentParticipants.forEach(p => { if (p.device_type) s.add(p.device_type); }); return Array.from(s).sort(); }, [currentParticipants]);
  ...
  const trackingAnalytics = useMemo(() => {
    ...
    return { countries: sortMap(countryCounts), cities: sortMap(cityCounts).slice(0, 10), devices: sortMap(deviceCounts), utmSources: sortMap(utmCounts), hasTrackingData: p.some(x => x.country || x.device_type || x.utm_source) };
  }, [currentParticipants]);
  ...
  const clearAllFilters = () => { setSearchQuery(''); setSegmentFilter('all'); setReasonFilter('all'); setDomainFilter(''); setCountryFilter(''); setCityFilter(''); setDeviceFilter(''); setDateFrom(''); setDateTo(''); };
  ...
  const filteredParticipants = useMemo(() => {
    ...
    if (searchQuery.trim()) { const q = searchQuery.toLowerCase(); list = list.filter(p => p.email?.toLowerCase().includes(q) || p.name?.toLowerCase().includes(q) || p.segment?.toLowerCase().includes(q) || p.sub_reason?.toLowerCase().includes(q) || p.country?.toLowerCase().includes(q) || p.city?.toLowerCase().includes(q) || p.ip_address?.includes(q)); }
    if (segmentFilter !== 'all') { if (variant === 'type') { list = list.filter(p => p.sub_reason === segmentFilter || p.afterfeel_type === segmentFilter); } else { list = list.filter(p => p.segment === segmentFilter); } }
    ...
    list.sort((a, b) => { const aVal = (a[sortField] || '').toLowerCase(); const bVal = (b[sortField] || '').toLowerCase(); if (aVal < bVal) return sortDir === 'asc' ? -1 : 1; if (aVal > bVal) return sortDir === 'asc' ? 1 : -1; return 0; });
    return list;
  ...
  const handleSort = (field: typeof sortField) => { if (sortField === field) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortField(field); setSortDir('asc'); } };
  ...
  const exportToCSV = () => {
    ...
    const link = document.createElement('a'); link.href=url; link.download=`piilk-v3-${variant}-${getNYCDate(0)}.csv`; link.click(); URL.revokeObjectURL(url);
  };
  ...
  const quizTypeCounts = useMemo(() => {
    ...
    currentParticipants.forEach(p => { const type = p.afterfeel_type || p.sub_reason || ''; if (Object.prototype.hasOwnProperty.call(counts, type)) counts[type]++; });
    return counts;
  ...
  const filteredAnalytics = useMemo(() => {
    ...
    if (analyticsPeriod === 'custom_range') { startDate = analyticsDateFrom || '2000-01-01'; endDate = analyticsDateTo || '2099-12-31'; }
      ...
      else { startDate = `${analyticsPeriod}-01`; const [y,m] = analyticsPeriod.split('-').map(Number); const lastDay = new Date(y,m,0); endDate = `${lastDay.getFullYear()}-${String(lastDay.getMonth()+1).padStart(2,'0')}-${String(lastDay.getDate()).padStart(2,'0')}`; }
    }
  ...
  const todayAnalytics = useMemo(() => {
    ...
    analyticsData.rawEvents.filter((ev:any)=>ev.d===todayStr&&ev.n==='page_view').filter((ev:any)=>!(ev.ip_address&&excludeIPs.some((ip:string)=>ev.ip_address.startsWith(ip)))).forEach((ev:any)=>{const vid=ev.v||ev.s; if(!vid)return; const isPaid=ev.um==='paid'; if(!uniqueVisitorMap.has(vid)) uniqueVisitorMap.set(vid,isPaid); else if(isPaid&&!uniqueVisitorMap.get(vid)) uniqueVisitorMap.set(vid,true);});
    ...
    const pVisitors=paidVids.size; const oVisitors=orgVids.size;
    const todayParticipants=currentParticipants.filter((p:any)=>toNYCDateStr(p.signed_up_at)===todayStr);
    ...
    return {visitors,sessions,submits,cvr,paid:{visitors:pVisitors,submits:pSubmits,cvr:pCvr},organic:{visitors:oVisitors,submits:oSubmits,cvr:oCvr}};
  }, [analyticsData, excludeIPs, currentParticipants]);

  const segColor = (s?: string, vTag?: 'main'|'type') => {
    const isType = vTag==='type'||variant==='type';
    if (isType) { const t=QUIZ_TYPE_LABELS[s||'']; if(t) return `bg-zinc-800/30 ${t.color} border-zinc-700/30`; }
  ...
  const segLabel = (s?: string, p?: Participant, vTag?: 'main'|'type') => {
    const isType = vTag==='type'||variant==='type';
    if (isType) { const type=p?.afterfeel_type||p?.sub_reason||s||''; const t=QUIZ_TYPE_LABELS[type]; return t?t.name:type||'afterfeel_quiz'; }
    switch(s) { case 'A': return 'Hot Leads'; case 'B': return 'Skeptic'; case 'C': return 'Newbie'; default: return s||'—'; }
  };

  const fmtDate = (d?: string) => {
    if(!d)return'—';
    try { const dt=new Date(d); return dt.toLocaleDateString('ko-KR',{year:'numeric',month:'2-digit',day:'2-digit'})+' '+dt.toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'}); } catch { return d; }
  };
  ...
  const trafficSourceLabel = (p: Participant): { label: string; color: string } => {
    const src=(p.utm_source||'').toLowerCase(); const med=(p.utm_medium||'').toLowerCase(); const ref=(p.referrer||'').toLowerCase();
    if (med==='paid'||src==='meta'||src==='facebook'||src==='fb') { if(src==='instagram'||ref.includes('instagram')) return {label:'IG',color:'bg-pink-500/20 text-pink-400 border-pink-500/30'}; return {label:'Meta',color:'bg-blue-500/20 text-blue-400 border-blue-500/30'}; }
  ...
  const deviceIcon = (d?: string) => { switch(d) { case 'mobile': return '📱'; case 'desktop': return '💻'; case 'tablet': return '📟'; default: return '—'; } };
    ...
    return (
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 sm:mb-5">
          <div className="flex items-center gap-2"><span className="text-base">📡</span><h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider">Traffic Source Breakdown</h3></div>
          <div className="flex gap-1 bg-zinc-800 rounded-lg p-0.5">
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <form onSubmit={handleAuth} className="text-center max-w-sm w-full">
          ...
          <p className="text-zinc-600 text-sm mb-6">Internal Dashboard <span className="text-blue-500 font-bold">v3</span></p>
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 text-zinc-400 text-sm cursor-pointer justify-start px-1">
              <input type="checkbox" checked={rememberMe} onChange={e=>setRememberMe(e.target.checked)} className="w-4 h-4 rounded border-zinc-700 bg-zinc-900 text-emerald-500 cursor-pointer" />
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center"><div className="w-10 h-10 border-2 border-zinc-800 border-t-blue-500 rounded-full animate-spin" /></div>;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-black to-zinc-900 text-white">
      ...
      <header className="sticky top-0 z-50 bg-black/90 backdrop-blur-xl border-b border-zinc-900">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <span className="text-[10px] text-zinc-600 uppercase tracking-wider hidden sm:inline">Dashboard</span>
            {/* ✅ V3 뱃지 */}
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 font-bold">v3</span>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        {/* View Mode Tabs — ✅ Meta 탭 추가 */}
        <div className="flex items-center gap-1 bg-zinc-900/60 border border-zinc-800 rounded-xl p-1">
          {(['overview','participants','analytics','meta'] as const).map(mode => (
            <button key={mode} onClick={() => setViewMode(mode)}
              className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-semibold transition-all capitalize
                ${viewMode === mode
                  ? mode === 'meta' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20'
            ...
            >
              ...
              {mode === 'meta'        && <span className="text-sm">📊</span>}
              {mode}
            </button>
          ))}
        </div>

        {/* Data Source Tabs (not for analytics/meta) */}
        {viewMode !== 'analytics' && viewMode !== 'meta' && (
          <div className="flex gap-2">
            <button onClick={() => setActiveSource('klaviyo')} className={`px-4 py-2 rounded-lg font-medium text-sm transition-all flex items-center gap-2 ${activeSource === 'klaviyo' ? 'bg-purple-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}>
              📧 Klaviyo {klaviyoData && <span className="text-xs opacity-80">({klaviyoData.total})</span>}
              {oppKlaviyoTotal !== null && <><span className="text-xs opacity-40">+</span><span className={`text-[10px] px-1.5 py-0.5 rounded-md font-bold border ${variant==='main'?'bg-purple-900/50 text-purple-300 border-purple-700/40':'bg-emerald-900/50 text-emerald-300 border-emerald-700/40'}`}>{oppositeVariant==='main'?'Main':'Quiz'} {oppKlaviyoTotal}</span></>}
            </button>
            <button onClick={() => setActiveSource('supabase')} className={`px-4 py-2 rounded-lg font-medium text-sm transition-all flex items-center gap-2 ${activeSource === 'supabase' ? 'bg-emerald-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}>
              🗄️ Supabase {supabaseData && <span className="text-xs opacity-80">({supabaseData.total})</span>}
              {oppSupabaseTotal !== null && <><span className="text-xs opacity-40">+</span><span className={`text-[10px] px-1.5 py-0.5 rounded-md font-bold border ${variant==='main'?'bg-purple-900/50 text-purple-300 border-purple-700/40':'bg-emerald-900/50 text-emerald-300 border-emerald-700/40'}`}>{oppositeVariant==='main'?'Main':'Quiz'} {oppSupabaseTotal}</span></>}
            </button>
          </div>
        )}
        ...
        {/* ══════ META ADS TAB ══════ */}
        {viewMode === 'meta' && (
          <MetaAdsSection currentParticipants={currentParticipants} variant={variant} />
        )}
            ...
            {trackingAnalytics && trackingAnalytics.hasTrackingData && (
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
                  {[{icon:'🌍',title:'Country',data:trackingAnalytics.countries,color:'bg-emerald-500'},{icon:'🏙️',title:'Top Cities',data:trackingAnalytics.cities,color:'bg-purple-500'},{icon:'📱',title:'Device',data:trackingAnalytics.devices,color:'bg-amber-500'},{icon:'🔗',title:'Traffic Source',data:trackingAnalytics.utmSources,color:'bg-sky-500'}].map(section => (
          ...
          <div className="space-y-4">
            ...
            {(() => {
              ...
              if (variant === 'type') {
                ...
                currentParticipants.filter(isToday).forEach(p => { const type=p.afterfeel_type||p.sub_reason||''; if(Object.prototype.hasOwnProperty.call(todayQuizCounts,type)) todayQuizCounts[type]++; });
                return (
              ...
              return (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                  <div className="relative bg-gradient-to-br from-zinc-800/80 to-zinc-900 border-2 border-zinc-600 rounded-xl p-4 overflow-hidden flex flex-col items-center justify-center text-center min-h-[160px]">
                    ...
                    <span className="text-lg sm:text-xl font-black text-emerald-400 mt-2">+{combinedTodayAll} today</span>
                  </div>
                  <div className="bg-emerald-950/30 border border-emerald-900/30 rounded-xl p-4 flex flex-col items-center justify-center text-center min-h-[160px]">
                    <p className="text-[10px] text-emerald-500 uppercase tracking-widest font-bold mb-2">Hot Leads Today</p>
                  ...
                  <div className="bg-sky-950/30 border border-sky-900/30 rounded-xl p-3 sm:p-5">
                    ...
                    {analyticsData ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs text-red-400 font-bold"><span className="w-2 h-2 rounded-full bg-red-500 inline-block"/>Paid</span><span className="text-2xl sm:text-3xl font-black text-white">{todayAnalytics.paid.visitors}</span></div>
                        <div className="flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs text-emerald-400 font-bold"><span className="w-2 h-2 rounded-full bg-emerald-500 inline-block"/>Organic</span><span className="text-2xl sm:text-3xl font-black text-white">{todayAnalytics.organic.visitors}</span></div>
                        <div className="flex items-center justify-between pt-1.5 border-t border-zinc-800"><p className="text-[10px] text-zinc-500 font-semibold">Total</p><p className="text-base font-black text-zinc-300">{todayAnalytics.visitors}</p></div>
                      </div>
                  ...
                  <div className="bg-purple-950/30 border border-purple-900/30 rounded-xl p-3 sm:p-5">
                    ...
                    {analyticsData ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs text-red-400 font-bold"><span className="w-2 h-2 rounded-full bg-red-500 inline-block"/>Paid</span><div className="flex items-baseline gap-1.5"><span className="text-[11px] text-zinc-500 font-mono">{todayAnalytics.paid.submits}/{todayAnalytics.paid.visitors}</span><span className="text-2xl sm:text-3xl font-black text-amber-400">{todayAnalytics.paid.cvr}</span></div></div>
                        <div className="flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs text-emerald-400 font-bold"><span className="w-2 h-2 rounded-full bg-emerald-500 inline-block"/>Organic</span><div className="flex items-baseline gap-1.5"><span className="text-[11px] text-zinc-500 font-mono">{todayAnalytics.organic.submits}/{todayAnalytics.organic.visitors}</span><span className="text-2xl sm:text-3xl font-black text-amber-400">{todayAnalytics.organic.cvr}</span></div></div>
                        <div className="flex items-center justify-between pt-1.5 border-t border-zinc-800"><p className="text-[10px] text-zinc-500 font-semibold">Avg</p><p className="text-base font-black text-purple-300">{todayAnalytics.cvr}</p></div>
                      </div>
                    ) : <p className="text-3xl font-black text-zinc-600">—</p>}
                  </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              <select value={segmentFilter} onChange={e=>setSegmentFilter(e.target.value)} className="px-4 py-2.5 bg-zinc-900 border border-zinc-800 rounded-xl text-white text-sm focus:outline-none focus:border-zinc-600 cursor-pointer min-w-[140px]">
                ...
                {variant==='type'?(<><option value="brick">🧱 Brick</option><option value="chalk">🦷 Chalk</option><option value="zombie">🧟 Zombie</option><option value="gambler">🎰 Gambler</option></>):(<><option value="A">A - Hot Leads</option><option value="B">B - Skeptics</option><option value="C">C - Newbies</option></>)}
              </select>
            ...
            {showAdvancedFilters && (
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 space-y-4">
                <div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-white">Advanced Filters</h3>{activeFilterCount>0&&<button onClick={clearAllFilters} className="text-xs text-purple-400 hover:text-purple-300">Clear all</button>}</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {[{label:'Reason',value:reasonFilter,onChange:(v:string)=>setReasonFilter(v),options:[{v:'all',l:'All Reasons'},...uniqueReasons.map(r=>({v:r,l:r}))]},{label:'Country',value:countryFilter,onChange:(v:string)=>setCountryFilter(v),options:[{v:'',l:'All Countries'},...uniqueCountries.map(c=>({v:c,l:c}))]},{label:'City',value:cityFilter,onChange:(v:string)=>setCityFilter(v),options:[{v:'',l:'All Cities'},...uniqueCities.map(c=>({v:c,l:c}))]},{label:'Device',value:deviceFilter,onChange:(v:string)=>setDeviceFilter(v),options:[{v:'',l:'All Devices'},...uniqueDevices.map(d=>({v:d,l:d}))]},{label:'Email Domain',value:domainFilter,onChange:(v:string)=>setDomainFilter(v),options:[{v:'',l:'All Domains'},...uniqueDomains.map(d=>({v:d,l:`@${d}`}))]}].map(f=>(
                    <div key={f.label}><label className="block text-[10px] text-zinc-500 uppercase tracking-widest mb-1.5">{f.label}</label><select value={f.value} onChange={e=>f.onChange(e.target.value)} className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white text-sm focus:outline-none focus:border-zinc-600 cursor-pointer">{f.options.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select></div>
                  ))}
            ...
            <div className="flex items-center justify-between">
              <p className="text-zinc-500 text-xs sm:text-sm">{filteredParticipants.length} participants{variant==='main'&&<span className="text-zinc-600 ml-1">(Main {currentParticipants.length} + Quiz {currentOtherParticipants.length})</span>}</p>
            </div>
            ...
            {participantsLoading ? (
              ...
              <div className="text-center py-16 text-zinc-600"><p className="text-sm">No participants found.</p></div>
            ) : (
                      ...
                      {filteredParticipants.map((p,i) => {
                        ...
                        return (
                          <tr key={p.id||i} className="border-b border-zinc-800/40 hover:bg-zinc-800/30 transition-colors">
                            ...
                            {variant==='main'&&<td className="px-2 py-3"><span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${vTag==='main'?'bg-emerald-500/20 text-emerald-400':'bg-purple-500/20 text-purple-400'}`}>{vTag==='main'?'Main':'Quiz'}</span></td>}
                            <td className="px-3 py-3 text-sm text-white font-medium max-w-[180px] truncate">{p.email}</td>
                            <td className="px-3 py-3"><span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-bold rounded-md border ${segColor(segKey,vTag)}`}>{segLabel(p.segment,p,vTag)}</span></td>
          ...
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={()=>setSelectedParticipant(null)}>
            <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto p-5 sm:p-6 space-y-4" onClick={e=>e.stopPropagation()}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{l:'Country',v:selectedParticipant.country},{l:'City',v:selectedParticipant.city},{l:'Device',v:selectedParticipant.device_type},{l:'UTM Source',v:selectedParticipant.utm_source},{l:'UTM Medium',v:selectedParticipant.utm_medium},{l:'UTM Campaign',v:selectedParticipant.utm_campaign},{l:'IP Address',v:selectedParticipant.ip_address},{l:'Afterfeel Type',v:selectedParticipant.afterfeel_type},{l:'Signed Up',v:fmtDate(selectedParticipant.signed_up_at)}].map(item=>(
        ...
        {viewMode === 'analytics' && (
          <div className="space-y-4 sm:space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {[{key:'all',label:'All'},{key:'today',label:'Today'},{key:'yesterday',label:'Yesterday'},{key:'last_7_days',label:'7D'},{key:'this_week',label:'This Week'},{key:'this_month',label:'This Month'},{key:'last_month',label:'Last Month'}].map(opt=>(
                ...
                {filteredAnalytics?.paidVsOrganic && (
                  <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 sm:p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {[{key:'paid' as const,label:'Paid',dot:'bg-red-500',bg:'bg-red-950/20 border-red-900/30',text:'text-red-400'},{key:'organic' as const,label:'Organic',dot:'bg-emerald-500',bg:'bg-emerald-950/20 border-emerald-900/30',text:'text-emerald-400'}].map(item=>(
                        <div key={item.key} className={`${item.bg} border rounded-xl p-4`}>
                          <div className="flex items-center gap-2 mb-3"><span className={`w-2.5 h-2.5 rounded-full ${item.dot}`}/><span className={`text-sm font-bold ${item.text}`}>{item.label}</span></div>
                          <div className="grid grid-cols-3 gap-3">
                            <div className="text-center"><p className="text-2xl font-black text-white">{filteredAnalytics.paidVsOrganic[item.key].views}</p><p className="text-[8px] text-zinc-500 uppercase mt-1">Views</p></div>
                            <div className="text-center"><p className="text-2xl font-black text-emerald-400">{filteredAnalytics.paidVsOrganic[item.key].submits}</p><p className="text-[8px] text-zinc-500 uppercase mt-1">Submits</p></div>
                            <div className="text-center"><p className={`text-2xl font-black ${Number(filteredAnalytics.paidVsOrganic[item.key].cvr)>0?'text-amber-400':'text-zinc-500'}`}>{filteredAnalytics.paidVsOrganic[item.key].cvr}%</p><p className="text-[8px] text-zinc-500 uppercase mt-1">CVR</p></div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            ) : <div className="text-center text-zinc-500 py-12">No analytics data yet.</div>}
          </div>
        )}

        <footer className="text-center pt-4 sm:pt-6 border-t border-zinc-900/50">
          <p className="text-[10px] sm:text-xs text-zinc-700">PIILK Internal v3 — Confidential · Meta API Live</p>
        </footer>
      </div>
    </main>
  );
