...
/* ─── 봇/크롤러 IP 차단 (DigitalOcean 서버 대역) ─── */
const BOT_IP_PREFIXES = [
  '209.38.', '64.23.', '137.184.', '146.190.',
  '24.199.', '134.199.', '147.182.', '165.225.',
  '143.110.', '176.3.', '172.56.',
];
...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const {
      event_name, event_data, session_id, visitor_id,
    ...
    // ✅ 봇 IP 차단 — Supabase에 아예 저장 안 함
    const ip = getClientIP(request);
    if (isBotIP(ip)) {
      return NextResponse.json({ success: true }); // 조용히 무시 (에러 반환 X)
    }
